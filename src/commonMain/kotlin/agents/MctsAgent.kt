...
data class TreeNode (val k: Double = 100.0){
    ...
    fun bestAction() : Int? {
        ...
        actions.forEach{(action,stats) ->
            run {
                val value = stats.mean() + epsilon * random.nextDouble()
                picker.add(value, action)
                // println("Action: $action, n = ${stats.n}, mean+R=${ value }")
            }
        }
