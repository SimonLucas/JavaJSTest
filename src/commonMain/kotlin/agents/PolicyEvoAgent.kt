...
data class PolicyEvoAgent(
...
) : SimplePlayerInterface, RolloutDataSource {
    ...
    private fun initialSequence(gameState: AbstractGameState): IntArray {
        if (random.nextDouble() < initUsingPolicy)
            return policySequence(gameState)
        else
    ...
    private fun policySequence(gameState: AbstractGameState): IntArray {
        val vp = policy
        // println(policy)
    ...
    private fun shiftLeftAndAppend(v: IntArray, gameState: AbstractGameState): IntArray {
