...
class StatSummary constructor(// following line can cause prog to hang - bug in Java?
    ...
    fun add(ss: StatSummary): StatSummary {
        ...
        sum += ss.sum
        sumsq += ss.sumsq
        max = kotlin.math.max(max, ss.max)
        min = kotlin.math.min(min, ss.min)
        valid = false
        return this
    }
    ...
    fun add(d: Double): StatSummary {
        ...
        sum += d
        sumsq += d * d
        min = kotlin.math.min(min, d)
        max = kotlin.math.max(max, d)
        valid = false
        return this
    }
